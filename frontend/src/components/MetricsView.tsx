...
const MetricsView = () => {
    ...
    // Get metrics by calculating them directly instead of using the getMetrics function
    // This prevents the infinite loop by removing the dependency on a function call
    const metrics = useMemo(() => {
        const userMsgs = messages.filter(m => m.sender === 'user').length;
        const botMsgs = messages.filter(m => m.sender === 'bot').length;

        // Calculate response times
        const botMessagesWithResponseTime = messages.filter(
            m => m.sender === 'bot' && m.responseTime !== undefined
        );

        const totalResponseTime = botMessagesWithResponseTime.reduce(
            (sum, msg) => sum + (msg.responseTime || 0), 0
        );

        const averageResponseTime = botMessagesWithResponseTime.length > 0
        ...
        <Box sx={{ width: '100%', maxWidth: 1200, mx: 'auto', p: 2 }}>
            ...
            >
                ...
                <Typography variant="body1" paragraph color="text.secondary">
                    Real-time analytics for your conversation with Reuben's Brews Chatbot.
                </Typography>
                    ...
                    <>
                        ...
                        <Box sx={{ height: 300, mb: 2 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    <CartesianGrid strokeDasharray="3 3" />
                                    <XAxis dataKey="id" label={{ value: 'Message Number', position: 'insideBottomRight', offset: -10 }} />
